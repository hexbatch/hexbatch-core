...
use Remotes\RemoteStackLogicType;
use Remotes\RemoteStackStatusType;
...
class RemoteStack extends Model
{
    ...
    {
        $what =  $this->belongsTo('Models\RemoteStack', 'parent_remote_stack_id');
        static::decorateBuilder($what);
        return $what;
    }

    public function children_activities() : hasMany {
        return $this->hasMany('Models\RemoteActivity','remote_stack_id','id')
            ->select('*')
    ...
    public function children_stacks() : hasMany {
        $what =  $this->hasMany('Models\RemoteStack','parent_remote_stack_id','id');
        static::decorateBuilder($what);
        return $what;
    }
